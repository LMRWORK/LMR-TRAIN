...
class AutoCompleteCity extends React.PureComponent  {
  ...
  constructor(props) {
    ...
    if (window.location.hash == '#search-city/from') {
      this.headText = 'from'
    } else if (window.location.hash == '#search-city/to') {
      this.headText = 'to'
    } else {
      this.headText = window.location.href
    }
  ...
  componentDidMount = () => {
    ...
    if (!this.props.stationsText) {
      ...
      fetch('/data/stations.txt').then((response) => response.text())
        .then((text) => {
  ...
  handleUpdateInput = (e) => {
    ...
    if (e.target.value) {
      ...
      while ((result = reg.exec(this.props.stationsText)) != null) {
        ...
        if (_filterStation.length>4) break;
      }
    }
...
const mapDispatchToProps = (dispatch) => ({fetchStationsText: (stationsText) => dispatch(fetchStationsText(stationsText))})
